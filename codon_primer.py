...
parser = argparse.ArgumentParser(description="Given a fasta file of coding "
                                             "sequences presents all possible"
                                             " PCR primers which preserve the"
                                             " coding sense of the sequence")
parser.add_argument('fasta',
                    help="path to fasta file containing "
                         "desired coding sequences")
parser.add_argument('-pl', type=int,
                    default=22,
                    help="desired length of primers, "
                         "default=22")
parser.add_argument('-f', type=int,
                    default=0,
                    help="number of base pairs flanking"
                         " coding region, "
                         "default = 0")
parser.add_argument('-o', type=str,
                    default="CDS_primers_out.txt",
                    help="specify output filename, "
                         "default = CDS_primers_out.txt")
parser.add_argument('-tmax', type=int, default=58,
                    help="maximum melting temp for primers to report, "
                    "default = 58")
parser.add_argument('-tmin', type=int, default=52,
                    help="minimum melting temp for primers to report, "
                    "default = 52")
parser.add_argument('-os',
                    default="single_rec_output.txt",
                    help="output table for unique primers, "
                    "default = single_rec_output.txt")
parser.add_argument('-om',
                    default="multi_rec_output.txt",
                    help="output table for non-unique primers, "
                         "default = multi_rec_output.txt")
...
for title, seq in SimpleFastaParser(seqhandle):
    ...
    for index in range(0, framelen - args.pl, 3):
        ...
        if Tm > args.tmin and Tm < args.tmax:
            ...
            if Hairpin.structure_found is False:
                outfile.write(title + "\tIn frame reverse\t" +
                              rcandidate + "\t" + str(Tm) + "\t" +
                              rcand_len + "\t" +
                              str(args.f + (framelen - index)) +
                              "\t" + str(framelen) + "\t" +
                              str(orientation) + "\n")
    ...
    for index in range(0, args.f - args.pl, 1):
        ...
        if Tm > args.tmin and Tm < args.tmax:
            Hairpin = primer3.calcHairpin(rcandidate)
            if Hairpin.structure_found is False:
                outfile.write(title + "\tPost frame reverse\t" +
                              rcandidate + "\t" + str(Tm) + "\t" +
                              rcand_len + "\t" +
                              str(args.f + framelen + (args.f - index)) +
                              "\t" + str(framelen) + "\t" +
                              str(orientation) + "\n")
